...
export default function MiCuenta() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      const { data } = await supabase
        .from('payment_agreements_transito')
        .select('*, payment_agreement_installments_transito(*)')
        .eq('user_id', user.id)
  ...
  const faltaAlgo = faltaDocumentos || faltaFacial
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 space-y-6">
      ...
      {faltaAlgo && (
        <div className="rounded-lg bg-amber-50 border border-amber-300 px-4 py-4 space-y-2">
          <p className="text-sm font-medium text-amber-800">
            Verificación pendiente antes de poder pagar comparendos:
          </p>
          <ul className="text-sm text-amber-700 space-y-1">
            {faltaDocumentos && <li>• Falta escanear el frente y reverso de tu cédula</li>}
            {faltaFacial && <li>• Falta registrar tu reconocimiento facial</li>}
          </ul>
          <Link to="/ajustes" className="btn-primary inline-block mt-2">
